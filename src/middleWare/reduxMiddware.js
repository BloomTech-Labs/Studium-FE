...
export const logger = store => next => action => {
  ...
  logOutMessageOrDebug(
    createMessage( "Dispatching", LOG_TYPES.INFO,
      REDUX_LOGGER_DEBUG_NAME,
    ) );
  
  logOutMessageOrDebug(
    createMessage( action, LOG_TYPES.OBJECT,
      REDUX_LOGGER_DEBUG_NAME,
    ) );
  ...
  logOutMessageOrDebug(
    createMessage( "Next State: ",
      LOG_TYPES.INFO, REDUX_LOGGER_DEBUG_NAME,
    ) );
  
  logOutMessageOrDebug(
    createMessage( store.getState(),
      LOG_TYPES.OBJECT, REDUX_LOGGER_DEBUG_NAME,
    ) );
...
export const storageBackUp = store => next => action => {
  ...
  if( action.type && action.type !== "SET_INIT_STATE" ){
    ...
    Object.keys( newState ).forEach( key => {
      ...
      logOutMessageOrDebug(
        createMessage( `Comparing local storage with new new
        state for reducer ${ key }`, LOG_TYPES.INFO,
          STORAGE_BACKUP_DEBUG_NAME,
        ) );
      
      logOutMessageOrDebug(
        createMessage( "PrevState:",
          LOG_TYPES.INFO, STORAGE_BACKUP_DEBUG_NAME,
        ) );
      
      logOutMessageOrDebug(
        createMessage( prevState,
          LOG_TYPES.OBJECT, STORAGE_BACKUP_DEBUG_NAME,
        ) );
      
      logOutMessageOrDebug(
        createMessage( "New State:",
          LOG_TYPES.INFO, STORAGE_BACKUP_DEBUG_NAME,
        ) );
      
      logOutMessageOrDebug(
        createMessage( newState,
          LOG_TYPES.OBJECT, STORAGE_BACKUP_DEBUG_NAME,
        ) );
      
      if( prevState !== newState ){
        logOutMessageOrDebug(
          createMessage( "Updating local storage.",
            LOG_TYPES.INFO, STORAGE_BACKUP_DEBUG_NAME,
          ) );
        ...
        logOutMessageOrDebug(
          createMessage( "State did not change.",
            LOG_TYPES.INFO, STORAGE_BACKUP_DEBUG_NAME,
          ) );
      }
    } );
