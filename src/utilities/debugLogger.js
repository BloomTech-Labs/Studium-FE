  ...
  if (process.env.REACT_APP_SHOW_LOGS || message.logType === LOG_TYPES.ERROR) {
    if (!message.location) {
      return logToConsoleTest(
        createMessage(
          'You must provide debug logger with a' +
...
const groupConsole = (message, debug) => {
  if (
    SYNAPS_CONFIG.appsToDebbug.includes(message.location) ||
    message.location === 'Debug Logger' ||
    ...
    if (currentGroup === null) {
      ...
      if (SYNAPS_CONFIG.groupLogsTogether) {
        ...
        console.group(`%c ${currentGroup}`, colotStr);
      }
...
export const createMessage = (
  ...
  if (location === 'unknown') {
    logOutMessageOrDebug(
      createMessage(
        'You must provider a location if you' +
          ' are trying to use the debug logger. Pass in message, logType, and' +
          ' location into the create message function.',
        LOG_TYPES.WARNING,
        'Debug Logger'
      )
    );
